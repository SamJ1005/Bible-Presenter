...
const PrelistVerseCard = ({
  ...
  useLayoutEffect(() => {
    ...
    while (
      (boxRef.current.scrollHeight > boxRef.current.clientHeight ||
       (tamilTextRef.current && tamilTextRef.current.scrollWidth > boxRef.current.clientWidth) ||
  ...
  const verseAreaPadding = isSmallMedium && isSingle
    ? '10px 58px 0'   // 0.5vh top, 3vw sides
    : '20px 19px 0';  // 1vh top, 1vw sides
  ...
  return (
    <div
      ...
      onClick={() => { if (!isEditing && !editingRefId) handleItemClick(item.id); }}
      style={{
        ...
        boxShadow: isActive ? '0 0 12px rgba(0,255,153,0.15)' : '0 2px 6px rgba(0,0,0,0.07)',
        marginBottom: '20px',
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ fontSize: '13px', fontWeight: 700, color: theme === 'dark' ? '#00ff99' : '#003399', opacity: 0.9 }}>
          {item.book} {item.chapter}:{item.verse}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <button type="button" onClick={(e) => handleFontSizeClick(-1, e)} style={btnStyle({ fontSize: '11px' })}>a-</button>
          <button type="button" onClick={(e) => handleFontReset(e)}         style={btnStyle({ fontSize: '12px' })}>↺</button>
          <button type="button" onClick={(e) => handleFontSizeClick(1, e)}  style={btnStyle({ fontSize: '13px' })}>A+</button>
          <div style={{ width: '1px', height: '16px', background: '#444', margin: '0 4px' }} />
          {!isEditing ? (
            <button type="button" onClick={(e) => { e.stopPropagation(); startEditingText(item); }}
              style={{ cursor: 'pointer', background: 'transparent', border: 'none', padding: '4px' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none"
                stroke={theme === 'dark' ? '#888' : '#555'} strokeWidth="2">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
              </svg>
            </button>
          ) : (
            <PrelistEditToolbar applyStyle={applyStyle} saveTextEdit={saveTextEdit}
              cancelTextEdit={cancelTextEdit} itemId={item.id} />
          )}
        </div>
      </div>
